...
# Install runtime deps as the unprivileged user from the start. Using
# `COPY --chown=...` for every COPY avoids a final `chown -R` over the
# (multi-million-file) node_modules tree, which on overlay filesystems can
# easily take 10+ minutes per layer.
...
RUN mkdir -p /app/tmp /app/sslcert \
    && chown masternode:masternode /app/tmp /app/sslcert
